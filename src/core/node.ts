...
export class Node {
  ...
  compilePath(): CompileResult {
    ...
    for (const w of walks) {
      ...
      // Def-family walk suppression (DEFECT #24 completion): a walk that passes
      // through a seam carrier via pure family hops (firstHopZone === undefined)
      // belongs to a def prototype subtree. The def delivers its authored truth
      // through the consumer's def-fill (emitDefRootElement/emitDefChildTree);
      // emitting standalone path-states would double-emit every link and
      // element inside the navBar/header/footer.
      if (firstHopZone(w) === undefined && w.hops.some((h) => h.owner.anchors.some((a) => a.role === 'child' && a.options.seam !== undefined))) {
        dropped.push({ arm: [this.id], reason: 'prototype-terminated' })
        continue
      }
  ...
  private materializeSeam(): void {
    ...
    for (const a of this.anchors) {
      ...
      if (seam === 'children' && defRoot !== undefined) {
        ...
        // NOTE: do NOT seam-wire protos to defRoot here. The def-root's own
        // family link from translate already delivers all child prototypes via
        // the standard family path. Adding a seam parent anchor for the same
        // children produces a phantom second route that duplicates every
        // nav/footer link in the compiled output (P-EMIT-8 double-path).
        // The protos loop is only needed when defRoot === undefined, handled
  ...
  private copyDefCss(defRoot: Node): void {
    const css = defRoot.css
    if (!css || Object.keys(css).length === 0) return
    const idx = this.layers.findIndex((l) => l.id === 'seam-css-def')
    const layer: NodeLayer = { id: 'seam-css-def', sourceName: 'css-seam', css: { ...css } }
    if (idx !== -1) this.layers[idx] = layer
    else this.layers.push(layer)
    this.compileLocal()
  }
