 ...
 * @processFlow Processed in Phase 7 (`ClientElementCreationWorker`/`SSRElementCreationWorker`) to output scoped CSS rules.
 */
export class Css {
   ...
   * @returns Target phase ID or undefined on property lock violation.
   */
   ...
   */
  public clone(ignoreProps: string[] = [], node?: Node, _actor: string = 'Css'): Css {
    const isAutoId = Boolean(
      node?.props?.isIdAutoGenerated || 
      (this.id && this.id.startsWith('preempt-node-'))
    );
    const cloned = new Css({
      id: (ignoreProps.includes('id') || isAutoId) ? undefined : this.id,
