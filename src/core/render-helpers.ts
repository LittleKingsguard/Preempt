...
export function emitElements(
  ...
  for (const s of actionable) {
    if (!isPathState(s) || !s.pathKey || !s.trace || s.trace.length < 2 || (s as { activePlacement?: string }).activePlacement === undefined) continue
...
function emitDefRootElement(
  ...
  const rootHandlers = (rootProto && 'handlers' in rootProto && Array.isArray((rootProto as any).handlers))
    ? (rootProto as any).handlers
    : (rootProto?.id && nodeById ? nodeById.get(rootProto.id)?.handlers : undefined) ?? []
...
function emitDefChildTree(
  ...
  const protoHandlers = (proto && 'handlers' in proto && Array.isArray((proto as any).handlers))
    ? (proto as any).handlers
    : (proto?.id && nodeById ? nodeById.get(proto.id)?.handlers : undefined) ?? []
  for (const h of protoHandlers) {
    if (h && typeof h === 'object' && typeof h.event === 'string') cprops[`on:${h.event}`] = true
  }
  const flat: MinimalElement[] = []
  const children: MinimalElement[] = []
