...
export async function logEvent(client: any, event: IPreemptEvent) {
  await client.query(
    `INSERT INTO Events (event_id, type, timestamp, source_id, source_type, interested_parties, state_change, correlation_id, version) 
    ...
    [
      ...
      event.correlationId, event.version
    ]
  );
}
