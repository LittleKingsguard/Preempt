...
export async function dbGetHandlerById(id: number) {
  ...
  const result = await queryFirstRow("SELECT id, name, body, author_id, is_approved, approved_roles, created_at, updated_at FROM Handlers WHERE id = $1", [id], "Handler not found");
...
export async function dbDeleteHandler(id: number) {
  const result = await queryFirstRow("DELETE FROM Handlers WHERE id = $1 RETURNING *", [id], "Handler not found");
  cache.clear();
  return result;
}
