...
- **Component Placement Rules & Anti-Patterns**:
  ...
  - **Anti-Pattern 3 (Self-Referential Component Loop)**: Defining a component whose sub-tree child nodes contain component bindings referencing the component itself (creating an infinite recursive component tree). `ComponentAssemblyWorker` and `SlotAssemblyWorker` execute `Component.isAppliedInAncestors(node, component)` during assembly, detecting native sub-tree component loops. *Exception*: Placements have their own infinite loop blocker (`Placement.constructor` safeguard). A component updating `type` or `children` can still resolve if an ancestor has applied it iff `node` is NOT descended strictly through native children (i.e. it crossed a placement boundary).
  - **Anti-Pattern 4 (Static Cache Keying Without Host Node)**: Searching or keying `Component.nodeCache` using the component reference string alone without host node pass-by-reference context. `Component.nodeCache` is structured as `Map<Node, Map<string, Node[]>>`, keying directly on the singular `hostNode` (`Node` object reference) to isolate component prototype nodes across different sub-trees.
