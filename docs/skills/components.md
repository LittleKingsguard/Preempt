...
- **Component Placement Rules & Anti-Patterns**:
  ...
  - **Anti-Pattern 4: Omitting Def-Root CSS on SED-1 Shell Collapse**: When a host node consumes a component via `target: "type"`, the host node collapses into the component root and adopts its children and phase handlers. Failing to copy the def-root's `css` (e.g. `classes` like `user-auth-dropdown` and `cssDef`) onto the consumer leaves the host node with empty CSS, breaking descendant query lookups (e.g. `container.findNode({ classes: ['...'] })` or `container.css.classes.includes(...)`).
