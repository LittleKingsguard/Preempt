...
- **Unsupported Anti-Patterns**:
  ...
  4. **Placement Packets Inside `template.root.children`**: Nodes declaring `targetPlacement` must never be authored inside `template.root.children`. They must reside in `template.children` (or `content[].content`), attaching to `contentNodes` to avoid dual family-and-placement walks that duplicate elements.
## Placement Emission & Phase Locking Rules
- **Non-Emitting Bypass**: Use `PhaseRegistry.EMIT_NONE` (from `PhaseRegistry`) to suppress auto-emissions when creating placement definitions on prototype/template nodes.
- **Target Placement Resolution (`targetPlacementResolution`)**: Target placement resolution emission (`Supervisor.emitToPhaseName(this, this.parent, 'targetPlacementResolution')`) executes on or before Phase 1 (`targetPlacementResolution`). Phase 1 locks on completion.
- **Placement Assembly (`placementAssembly`)**: Placement assembly emission (`Supervisor.emitToPhaseName(this, this.parent, 'placementAssembly')`) executes during assembly phases (on or before Phase 5 `slotAssembly`) and respects Phase 2 (`placementAssembly`) locking.
