...
describe('Content Pipeline Integration', () => {
  ...
  it('cascades payload updates through the pipeline and replaces referencing nodes cleanly', async () => {
    ...
    await Supervisor.process({ runInstantiation: true, runAssembly: true, runMonitoring: true, runPreprocessing: false, runValidation: false, runRendering: false, runPostprocessing: false }, template);
    
    // 2. Fetch/inject initial content payload targeting the placement
    const payload1 = new Payload({
    ...
    expect(articleNodes[0]?.content?.toString()).toBe('Version 1');
    ...
    const updatedNodes = Supervisor.getContentNodes().filter(n => n.type === 'article');
    expect(updatedNodes.length).toBe(1);
    expect(updatedNodes[0]?.content?.toString()).toBe('Version 2'); // Updated content is present
